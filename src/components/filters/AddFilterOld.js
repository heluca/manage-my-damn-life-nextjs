...
import { getI18nObject, isDateValid } from "@/helpers/frontend/general"
...
class AddFilters extends Component {
    ...
    constructor(props) {
        ...
        this.removeDanglingLabelsFromIncomingFilters = this.removeDanglingLabelsFromIncomingFilters.bind(this)
        ...
        if (this.props.mode == "edit" && this.props.mode != null && this.props.mode != undefined) {
            ...
            if (this.props.filter != null && this.props.filter != undefined) {
                ...
                if (filterDueIsValid(this.props.filter.filter.due)) {
                    ...
                    // console.log("this.props.filter.filter.due[0]", this.props.filter.filter.due)
                    var dueFrom = isDateValid(this.props.filter.filter.due[0]) ?  new Date(this.props.filter.filter.due[0]) : new Date()

                    var dueBefore = isDateValid(this.props.filter.filter.due[1]) ?   new Date(this.props.filter.filter.due[1]) : new Date(Date.now())
                ...
                if (this.props.filter.filter.label != null && this.props.filter.filter.label != undefined && Array.isArray(this.props.filter.filter.label) && this.props.filter.filter.label.length > 0) {
                    ...
                    // We need to remove dangling labels.
                    this.removeDanglingLabelsFromIncomingFilters(this.props.filter.filter.label)
    ...
    async removeDanglingLabelsFromIncomingFilters(labelsFromProps) {
        if (!isValidResultArray(labelsFromProps)) {
            return
        }
        var labels = await getAllLabelsFromDexie()
        // console.log("labelsFromProps", labelsFromProps, labels)
        let newLabelArray = []
        for (const k in labelsFromProps) {
            for (const i in labels) {
                if (labels[i].name == labelsFromProps[k]) newLabelArray.push(labelsFromProps[k])
            }
        }
        // console.log("filter", newLabelArray)
        this.setState((prevState) => {
            let newFilters = _.cloneDeep(prevState.selectedFilters)
            // console.log("newFilters", newFilters)
            newFilters.filter["label"] = newLabelArray
            return {
    ...
    render() {
        ...
        // let dueFrom = this.state.dueDateFrom 
        // let dueBefore = this.state.dueDateBefore
        
        // console.log("dueFrom", this.state.dueDateFrom, dueFrom, "due Before", this.state.dueDateBefore, dueBefore)
